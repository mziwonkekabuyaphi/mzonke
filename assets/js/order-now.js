...
new Vue({
    ...
    computed: {
        ...
        filteredProducts() {
            ...
            if (this.activeCategory !== 'all') {
                if (this.activeCategory === 'spirits') {
                    list = list.filter(p => p.category === this.activeSub);
                } else {
    ...
    getQty(id) {
        return this.pendingQtys[id] || 0;
    },

    incrementQty(p) {
        this.$set(this.pendingQtys, p.id, (this.pendingQtys[p.id] || 0) + 1);
    },

    decrementQty(p) {
        let q = this.pendingQtys[p.id] || 0;
        if (q > 0) this.$set(this.pendingQtys, p.id, q - 1);
    },
    ...
    getProductImage(product) {
        return (product.image && product.image.trim()) ? product.image : 'https://placehold.co/300x200?text=No+Image';
    },
    ...
    addToCart(p) {
        ...
        if (existing) {
            ...
            this.cart.push({ ...p, quantity: qty, vendor, product_type });
        }
    ...
    async startCheckout() {
        ...
        if (this.walletBalance < total) {
            this.showToast(`Insufficient balance. Available: R${this.formatPrice(this.walletBalance)}`);
            return;
        }
    ...
    async processOrderDeductionAndCreate(scheduledFor = null) {
        ...
        const { data: newBalance, error: deductError } = await supabase.rpc('deduct_wallet_balance', {
        ...
        this.pendingCartSnapshot.forEach(item => {
            if (!ordersByVendor[item.vendor]) ordersByVendor[item.vendor] = [];
            ordersByVendor[item.vendor].push(item);
        });
    ...
    async collectNow() {
        try {
            await this.processOrderDeductionAndCreate(null);
        } catch (err) {
    ...
    async scheduleLater() {
        if (!this.selectedSlot) {
            this.showToast('Select a time slot');
            return;
        }
        const now = new Date();
        ...
        try {
            await this.processOrderDeductionAndCreate(scheduledDate.toISOString());
            this.selectedSlot = null;
        } catch (err) {
            this.showToast(`Scheduling failed: ${err.message}`);
            await this.fetchWalletBalance();
        }
    },
    ...
    async renderTrackModal() {
        ...
        for (const order of activeOrders) {
            ...
            ordersHtml += `
                <div class="order-card ${order.status === 'ready' ? 'ready-glow' : ''}">
                    <div class="card-header">
                        <span>#${order.order_number}</span>
                        <span><i class="fas ${order.vendor === 'Rands Smart Counter' ? 'fa-beer-mug-empty' : 'fa-utensils'}"></i> ${order.vendor === 'Rands Smart Counter' ? 'Bar' : 'Kitchen'}</span>
                        <span>${order.status.toUpperCase()}</span>
                    </div>
                    <div class="timeline">
                        <div class="step-row">
                            ${steps.map((s, idx) => `
                                <div class="step ${idx < activeIdx ? 'completed' : (idx === activeIdx ? 'active' : '')}">
                    ...
                    <div style="padding:12px;">
                        ${itemsList}
                        <div style="font-weight:bold; margin-top:8px; color:var(--red);">R${order.total.toFixed(2)}</div>
                    </div>
                </div>
            `;
        }
        ...
        if (scheduled.length) {
            bookedHtml = `
                <div style="padding:12px 20px;">
                    <h3 style="display:flex; gap:8px;"><i class="fas fa-calendar-alt" style="color:var(--red);"></i> Scheduled Orders</h3>
                </div>
                    ...
                    ${scheduled.map(b => {
                        ...
                        return `
                            <div class="order-card">
                                ...
                                <div style="padding:12px;">
                                    ${itemsList}
                                    <div style="font-weight:bold; margin-top:8px; color:var(--red);">R${b.total.toFixed(2)}</div>
                                </div>
                            </div>
                        `;
                    }).join('')}
    ...
    startPollingOrders() {
        setInterval(() => {
            if (this.trackModalVisible) this.renderTrackModal();
        }, 3500);
    },
