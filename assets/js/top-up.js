...
new Vue({
    ...
    async loadUserBalance() {
        ...
        const { data: wallet, error: walletErr } = await supabase.from('wallets').select('balance').eq('user_id', session
            .user.id).maybeSingle();
    ...
    async completeTopup(amount) {
        const { data: { session } } = await supabase.auth.getSession();
        if (!session) return;
        const { data: wallet, error: fetchErr } = await supabase.from('wallets').select('balance').eq('user_id', session
            .user.id).single();
        if (fetchErr) return;
    ...
    setupRealtimeListener() {
        this.realtimeChannel = supabase.channel('topup-approvals').on('postgres_changes', {
            ...
            if (approved.reference === `TU-${this.previewRef}` && this.waitingModalOpen) {
                ...
                const { data: { session } } = await supabase.auth.getSession();
                if (session) {
                    const { data: wallet } = await supabase.from('wallets').select('balance').eq('user_id', session.user
                        .id).single();
    ...
    async startYocoPayment() {
        ...
        try {
            ...
            if (insertError || !pendingTx || !pendingTx.id) {
            ...
            if (edgeError) {
                ...
                await supabase
                    .from('wallet_transactions')
                    .update({ status: 'failed', metadata: { error: edgeError.message || 'Edge function failed' } })
                    .eq('id', transactionId);
                throw new Error(edgeError.message || 'Payment gateway temporarily unavailable');
            }

            if (!edgeResponse || typeof edgeResponse !== 'object') {
                console.error('Invalid edge response (empty/malformed):', edgeResponse);
                await supabase
                    .from('wallet_transactions')
                    .update({ status: 'failed', metadata: { error: 'Invalid gateway response' } })
                    .eq('id', transactionId);
                throw new Error('Invalid response from payment gateway');
            }

            const checkoutUrl = edgeResponse.checkout_url || edgeResponse.checkoutUrl;

            if (!checkoutUrl || typeof checkoutUrl !== 'string' || checkoutUrl.trim() === '') {
                console.error('Missing or invalid checkout URL in response:', edgeResponse);
                await supabase
                    .from('wallet_transactions')
                    .update({ status: 'failed', metadata: { error: 'Missing checkout URL', rawResponse: edgeResponse } })
                    .eq('id', transactionId);
                throw new Error('Payment gateway error: missing checkout URL');
            }
            ...
            this.showToast(err.message || 'Payment initiation failed. Please try again.', 'error');
            this.yocoProcessing = false;
        }
    }
});