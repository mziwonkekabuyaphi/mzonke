...
// ===== DOM References =====
// These are initialized in init() after DOM is ready
let dom = {};
...
async function showVVIPInfo(title, message) {
    ...
    const onConfirm = () => {
        ...
        document.getElementById('vvipModalConfirmBtn').removeEventListener('click', onConfirm);
    };
...
function renderIntegratedPackage() {
    ...
    if (!activePackage || !activePackage.package) {
        container.innerHTML =
            `<div class="no-package-message"><i class="fas fa-crown"></i><p>No active VVIP package</p><p style="font-size:0.7rem;">Visit VVIP Tables to purchase a package</p></div>`;
...
async function loadUserAndPackageCredit() {
    ...
    const { data: profile } = await supabase.from('profiles').select('id, name, phone, role').eq('id', currentUser.id)
        .maybeSingle();
    if (!profile) {
        ...
        currentProfile = { id: currentUser.id, name: currentUser.user_metadata?.full_name || 'Member', phone: '',
        role: 'user' };
    } else { currentProfile = profile; }

    const { data: wallet } = await supabase.from('wallets').select('balance, status, block_reason').eq('user_id',
...
async function loadActivePackageAndCredit() {
    ...
    const { data: booking } = await supabase
        .from('vvip_bookings')
        .select(
            `id, booking_reference, total_amount, remaining_balance, used_amount, status, table_id, packages:vvip_packages(name, price, description, capacity, benefits), table:vvip_tables(table_number)`
            )
...
async function loadPackages() {
    ...
    if (!data?.length) {
        container.innerHTML =
        '<div class="loading-state"><i class="fas fa-crown"></i><p>No VVIP Tables available</p></div>';
        return;
    }

    const packageIds = data.map(p => p.id);
    const { data: allTables } = await supabase.from('vvip_tables').select('id, package_id, table_number').in('package_id',
        packageIds);
    const { data: activeBookings } = await supabase.from('vvip_bookings').select('table_id').not('status', 'in',
        '(cancelled,no_show)').not('table_id', 'is', null);
...
// ===== Window-scoped functions for inline onclick =====
window.selectPackage = async function(pkg) {
    ...
    if (currentTabCredit > 0) {
        showToastMsg(`Cannot purchase - you already have an active package with R${currentTabCredit} remaining. Use that first.`,
            true);
        return;
    }
    selectedPackage = pkg;
    ...
    const { data: tables } = await supabase.from('vvip_tables').select('id, table_number').eq('package_id', pkg.id).order(
        'table_number');
    const { data: activeBookings } = await supabase.from('vvip_bookings').select('table_id').not('status', 'in',
        '(cancelled,no_show)').not('table_id', 'is', null);
    const bookedTableIds = new Set((activeBookings || []).map(b => b.table_id));
...
async function confirmPurchase() {
    ...
    if (isWalletBlocked) { showToastMsg(walletBlockReason, true);
        document.getElementById('purchaseModal').classList.remove('active'); return; }
    ...
    await supabase.from('vvip_packages').update({ sold_count: (selectedPackage.sold_count || 0) + 1 }).eq('id',
        selectedPackage.id);
    showToastMsg(
        `Package purchased! Ref: ${bookingRef}${selectedTableLabel ? ' | Table: ' + selectedTableLabel : ''} | Credit: R${tabCredit.toFixed(2)}`
        );
...
async function init() {
    ...
    document.getElementById('printVVIPBandBtn')?.addEventListener('click', printVVIPBand);
    document.getElementById('requestServiceBtn')?.addEventListener('click', openServiceRequestModal);
    document.getElementById('submitServiceRequest')?.addEventListener('click', sendServiceRequest);
    document.getElementById('cancelServiceRequest')?.addEventListener('click', () => document.getElementById(
        'serviceRequestModal').classList.remove('active'));
    document.getElementById('purchaseModalCancelBtn')?.addEventListener('click', () => document.getElementById(
        'purchaseModal').classList.remove('active'));
    document.getElementById('purchaseModalConfirmBtn')?.addEventListener('click', confirmPurchase);
    document.getElementById('storeTabBtn')?.addEventListener('click', switchToStore);
    document.getElementById('wristbandTabBtn')?.addEventListener('click', switchToWristband);
    document.getElementById('howToPayBtn')?.addEventListener('click', async () => {
