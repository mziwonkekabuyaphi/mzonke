    ...
    let availableTablesForSelection = []; // tables (with .taken flag) for the package open in the purchase modal
    let eventsById = {}; // cached events, keyed by id, for showing each package's date
    ...
    async function loadActivePackageAndCredit() {
        ...
        if (booking) {
            ...
            activePackage = {
                ...
                pricePaid: booking.total_amount,       // what the customer paid for the table
                creditIssued: remaining + used,          // the tab credit they started with
                remainingAmount: remaining,              // tab credit still available to spend
                usedAmount: used,
    ...
    async function loadPackages() {
        ...
        // Real table + booking data so availability reflects actual named tables, not just a stored count
        ...
        // Event dates, since packages are tied to a specific event
    ...
    window.selectPackage = async function(pkg) {
        ...
        // Pull this package's tables fresh (right at click time) so the list is as current as possible
    ...
    async function confirmPurchase() {
        ...
        // Guard against two people booking the same table at the same moment
        if (selectedTableId) {
    ...
    async function init() {
        ...
        document.getElementById('cancelServiceRequest')?.addEventListener('click', () => document.getElementById('serviceRequestModal').classList.remove('active'));
        document.getElementById('purchaseModalCancelBtn')?.addEventListener('click', () => document.getElementById('purchaseModal').classList.remove('active'));
