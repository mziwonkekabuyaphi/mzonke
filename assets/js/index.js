...
// FIX: Helper to safely enable/disable the install button
...
// FIX: beforeinstallprompt — fires when browser decides PWA is installable
window.addEventListener(‘beforeinstallprompt’, (e) => {
e.preventDefault(); // stop mini-infobar on mobile
deferredPrompt = e;

// Only show banner if not previously dismissed
if (!localStorage.getItem(‘installDismissed’) && installBanner) {
installBanner.style.display = ‘flex’;
setInstallReady(true); // now safe to click
}

console.log(‘📲 Install prompt captured and ready’);
});

// FIX: Install button click — always guard against null deferredPrompt
...
// FIX: Always clean up after prompting, regardless of outcome
deferredPrompt = null;
...
// FIX: Once installed, clean up banner and prompt
...
// FIX: If app is already running in standalone mode, never show install banner
