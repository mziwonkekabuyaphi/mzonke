// Primary path: reuse the dashboard's authenticated client via
// window.parent.supabase (see initSupabase below).
//
// Fallback path only: Supabase is loaded via a dynamic import (instead of
// a static top-level `import { supabase } from ...`) because a static
// import that can't resolve a named export throws immediately and
// silently kills the whole module — Vue never mounts and every feature
// (not just Add Product) stops working. Doing it dynamically, with a
// fallback for either a default or named export, means a missing/
// mismatched config file degrades gracefully instead of taking the whole
// app down. Note this fallback client is unauthenticated (anon) — writes
// will fail RLS unless it's later given its own login flow.
let supabase = null;

async function initSupabase() {
  // This page is loaded in an iframe inside dashboard.html, same-origin.
  // The dashboard already has an authenticated (staff/admin) Supabase
  // session; a fresh client created from config/supabase.js in here would
  // start out as `anon`, since there's no separate login flow on this page.
  // Reuse the dashboard's own client object directly — same-origin frames
  // can read each other's window properties, so this picks up the exact
  // same session (and stays in sync if it refreshes or the user logs out),
  // no token copying or postMessage handshake needed.
  try {
    ...
    // Cross-origin parent would throw a SecurityError here — fall through
    // to the standalone client below.
    console.warn('Could not access parent window Supabase client:', err);
  }

  // Fallback for standalone use (e.g. opening this page directly, outside
  // the dashboard iframe, for local testing) — this client will only be
  // able to do anon-permitted reads until it has its own login flow.
  try {
...
createApp({
  ...
  async mounted() {
    ...
    if (this.supabase) {
      ...
      this.showToast('Could not connect to Supabase. Not running inside the dashboard iframe and ../../config/supabase.js is missing/invalid.', 'error');
    }
  },
