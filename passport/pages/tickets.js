...
// user navigates away and back to this page within the SPA session.
let cleanup = [];
const onCleanup = (fn) => cleanup.push(fn);

// currentUser/userProfile/events/etc. are intentionally module-scope (not
// re-declared inside init()) so they survive a navigate-away-and-back —
// dynamic import() caches the module, this file only ever loads once per
// app session. Same tradeoff as appState in state.js. NOTE: this page still
// does its own auth/profile/wallet fetch (incl. the profile auto-create
// fallback) instead of reading appState from state.js — that's pre-existing
// duplication carried over from the old multi-page setup, not something
// this conversion pass changes. Worth consolidating later (see MIGRATION.md).
let currentUser = null, userProfile = null, events = [], selectedEventId = null,
    currentTicketTypes = [], transferTicketId = null, isPurchasing = false,
    isWalletBlocked = false, walletBlockReason = '', walletChannel = null;
...
function wireStaticListeners() {
    ...
    bind('confirmTransferBtn', 'click', async () => {
        const phone = document.getElementById('transferPhone').value.trim();
        if (phone && transferTicketId) { await transferTicketToPhone(transferTicketId, phone); document.getElementById('transferModal').classList.remove('show'); }
        else showToast('Enter phone number', true);
    });
...
export default {
    async init() {
        ...
        walletChannel = supabase
            ...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'wallets', filter: `user_id=eq.${userProfile.id}` }, (payload) => {
